...
const AddIngreso = () => {
  ...
  useEffect(() => {
    document.title = "Agregar ingreso | Gestor de gastos";
  }, [])

  const inputClass = (field) =>
    `w-full pl-10 pr-4 py-2.5 border rounded-xl text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-400 ${errors[field] ? "border-red-400" : "border-gray-200"
    }`;
    ...
    <div className="p-6 flex items-start justify-center" style={{ background: "#f0f4ff", minHeight: "100%" }}>
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 w-full max-w-lg p-8 mt-4">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-800">Registrar Ingreso</h1>
            <p className="text-gray-500 text-sm">Completá los datos del ingreso</p>
          </div>
        </div>
        ...
        {success && (
          <div className="flex items-center gap-2 bg-green-50 border border-green-200 text-green-600 text-sm rounded-xl px-4 py-3 mb-4">
            <CheckCircle size={16} /> ¡Ingreso registrado exitosamente!
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input type="number" name="monto" value={formData.monto}
                onChange={handleChange} placeholder="Ej: 50000"
          ...
          <div>
            ...
            <div className="relative">
              ...
              <textarea name="descripcion" value={formData.descripcion}
                onChange={handleChange} placeholder="Describí el ingreso..." rows={3}
                className={`w-full pl-10 pr-4 py-2.5 border rounded-xl text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-400 resize-none ${errors.descripcion ? "border-red-400" : "border-gray-200"}`}
              />
            </div>
          ...
          <button type="submit" disabled={loading}
            className="w-full bg-gradient-to-r from-green-500 to-emerald-600 text-white font-bold py-3 rounded-xl shadow-md hover:opacity-95 transition-all disabled:opacity-50">
